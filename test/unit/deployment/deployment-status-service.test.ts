...
describe('DeploymentStatusService', () => {
  ...
  it('refreshes remote in-progress status and clears stale failed wave', async () => {
    await stateManager.saveState({
      deploymentId: '0AfRemoteStatus',
      targetOrg: 'test@example.com',
      timestamp: '2026-04-20T00:00:00.000Z',
      totalWaves: 2,
      completedWaves: [],
    ...
    const summary = await service.getStatus({ refreshRemote: true, targetOrg: 'test@example.com' });
    const refreshed = await stateManager.loadState();
